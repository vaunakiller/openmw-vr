...
namespace MWVR {
    ...
    {
    public:
        static constexpr XrStructureType XrStructureType = XR_TYPE_SWAPCHAIN_IMAGE_OPENGL_KHR;
    ...
    public:
        ...
        void blit(osg::GraphicsContext* gc, VRFramebuffer& readBuffer) override
        {
            mFramebuffer->bindFramebuffer(gc, GL_FRAMEBUFFER_EXT);
            readBuffer.blit(gc, 0, 0, mFramebuffer->width(), mFramebuffer->height(), mBufferBits, GL_NEAREST);
    ...
    {
    public:
        static constexpr XrStructureType XrStructureType = XR_TYPE_SWAPCHAIN_IMAGE_D3D11_KHR;
    ...
    public:
        ...
        void blit(osg::GraphicsContext* gc, VRFramebuffer& readBuffer) override
        {
            ...
            mFramebuffer->bindFramebuffer(gc, GL_FRAMEBUFFER_EXT);
            readBuffer.blit(gc, 0, mFramebuffer->height(), mFramebuffer->width(), 0, mBufferBits, GL_NEAREST);
            xr->impl().platform().DXUnlockObject(mDxResourceShareHandle);
            readBuffer.bindFramebuffer(gc, GL_FRAMEBUFFER_EXT);
            

            // If the d3d11 texture couldn't be shared directly, add another redudant blit.
            if (mSharedTexture)
            {
        ...
        enumerateSwapchainImagesImpl(osg::GraphicsContext* gc, XrSwapchain swapchain, XrSwapchainCreateInfo swapchainCreateInfo)
    {
        ...
        images.resize(imageCount, { SwapchainImage::XrStructureType });
