...
namespace Stereo
{
    ...
    //! Create a framebuffer attachment from the specified opengl texture handle. Target must be GL_TEXTURE_2D or GL_TEXTURE_2D_ARRAY. If the target is GL_TEXTURE_2D_ARRAY, the specified layer will be attached.
    //! The layer argument is ignore if target is GL_TEXTURE_2D.
    osg::FrameBufferAttachment createLayerAttachmentFromHandle(osg::State* state, uint32_t handle, uint32_t target, uint32_t width, uint32_t height, uint32_t layer);
